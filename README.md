...
a complete interpretability stack. Methods are chosen per-architecture:

| Architecture | Method | Output |
|---|---|---|
| CatBoost, XGBoost | **SHAP TreeExplainer** + native importance | `feature_importance.csv`, SHAP beeswarm plots |
| CANN, CANN-GBM, FT-T, TabM, LocalGLMnet, DRN | **Captum Integrated Gradients** (50 steps) | per-continuous-feature attributions |
| FT-Transformer | **Per-layer attention extraction** | `fig_dl_06_attention_heatmap.png` |
| CANN, CANN-GBM | **Residual analysis** | NN-correction distribution vs base prediction |
| LocalGLMnet | **Per-row coefficient extraction** | `localglmnet_coefficients.csv` (one row per test record) |
| DRN | **Distributional outputs** | `drn_distributional_outputs.csv` (mean, variance, quantiles, VaR95, VaR99) |

The pre-trained collections on HF ship all these artefacts as v3:

- 📊 [`dashboard_dl_interpretability.html`](https://huggingface.co/t22000t/house-prices-tabular-models/blob/main/dashboard_dl_interpretability.html) - interactive Plotly dashboard combining all methods
- 📄 `INTERPRETABILITY.md` - human-readable summary report (generated by [`scripts/summarise_interpretability.py`](scripts/summarise_interpretability.py))
- 📈 Static publication figures in `figures/`

**Headline cross-method findings** (features that appear in top-5 across both CatBoost SHAP and XGBoost gain on each dataset):

| Dataset | Consensus top features | Note |
|---|---|---|
| House Prices | `TotalSF` (derived), `OverallQual`, `GrLivArea`, `YearRemodAdd` | The derived `TotalSF` feature out-ranks each of its components - validating the `derived_features` mechanism in `DatasetConfig`. |
| Bike Sharing | `hr`, `yr`, `temp`, `workingday`, `season` | `hr` dominates with importance ~5× the next feature. Models that fail to lean heavily on `hr` (e.g. FT-Transformer, TabM) underperform. |

Generate a summary report for your own runs:
```bash
python scripts/summarise_interpretability.py results/your_8arch_dir
```
